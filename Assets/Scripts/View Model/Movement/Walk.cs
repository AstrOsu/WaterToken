...
public class Walk : Movement 
{
	...
	public override IEnumerator Traverse (Tile tile)
	{
		...
		for (int i = 1; i < targets.Count; ++i)
		{
			...
			if (from.height == to.height)
				yield return StartCoroutine(WalkTo(to));
			else
				yield return StartCoroutine(Jump(to));
		}
		yield return null;
	}
	IEnumerator WalkTo (Tile target)
	{
		Tweener tweener = transform.MoveTo(target.center, 0.1f, EasingEquations.Linear);
		while (tweener != null)
			yield return null;
	}
	IEnumerator Jump (Tile to)
	{
		...
		//Debug.Log(tweener.easingControl.duration);
		Tweener t2 = jumper.MoveToLocal(new Vector3(0, Tile.step * 2f, 0), tweener.easingControl.duration / 2f, EasingEquations.EaseOutQuad);
